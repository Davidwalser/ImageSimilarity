...
\section{Sift}
\subsection{Sift Implementierung}
\label{sec:sift_implementation}
Die SIFT Implementierung stellte sich als schnelle und einfache (ca. 50 Zeilen Code) Lösung dar.
\begin{lstlisting}[frame=lines, caption=SIFT Implementierung, captionpos=b, label = lst:sift_implementation, language=Python, showstringspaces=false]
def get_sift_similarity_score(filename1, filename2, plot_result = False):
    try:
        ...
        return 0
\end{lstlisting}
Die Funktion \textit{get\_sift\_similarity\_score} nimmt drei Parameter entgegen.
Die ersten beiden Parameter sind \textit{filename1} und \textit{filename2}, welche die Dateinamen für die zu vergleichenden Bilder sind.
Der dritte Parameter ist optional, heißt \textit{plot\_result} und ist per default auf \textit{False} gesetzt.
Die komplette Funktion befindet sich in einem \textit{try catch} Block, da bei einem Fehler davon ausgegangen wird dass die Bilder sich nicht ähnlich sind.
Zuerst werden die zwei Bilder eingelesen und ein SIFT Objekt instanziert.
Mithilfe der Funktion \textit{sift.detectAndCompute} werden die Keypoints und Deskriptoren für beide Bilder ermittelt.
Anschließend werden die Matches berechnet (siehe Listing \ref{lst:sift_calculateMatches}).
Wenn der Parameter \textit{plot\_result} auf \textit{True} gesetzt wurde, so werden die gefundenen Keypoints und die dazugehörigen Matches grafisch ausgegeben.
Zuletzt wird der Ähnlichkeitswert mithilfe der Anzahl gefundenen Matches dividiert durch das Minimum der gefundenen Keypoints der beiden Bilder berechnet.
Das Minimum wird benötigt, da es maximal soviele Matches geben kann wie Keypoints.

Die Funktion zur Bestimmung der Matches nimmt die zwei Deskriptoren als Parameter entgegen.
Mithilfe des Brute Force Matchers und der \textit{knnMatch} Funktion werden aus den Deskriptoren die Matches ermittlet.
Aus diesen Matches werden die potentiell korrekten Matches extrahiert indem die Distanz der Matches verglichen wird. 
Laut des Papers von David Lowe \cite{siftlowe} sind es korrekte Matches, wenn die Distanz der Matches kleiner einem Ratio von 0.7 entspricht.
Anschließend wird überprüft ob der beste Match für Deskriptor1 verglichen mit Deskriptor2 derselbe wie für Deskriptor2 verglichen mit Deskriptor1 ist, um so nur bidirektionale Matches zu haben.
\begin{lstlisting}[frame=lines, caption=SIFT calculateMatches Funktion, captionpos=b, label = lst:sift_calculateMatches, language=Python, showstringspaces=false]
...
\subsection{Ergebnise}
Um SIFT zu testen wurde eine Hilfsfunktion (siehe Listing \ref{lst:sift_test_function}) geschrieben, mithilfe der aus einem Pool an Bildern die Bilder mit dem höchsten SIFT Score ermittelt und ausgegeben werden können.
Die Parameter der Funktion sind \textit{testImage\_path}, was der Pfad für das zu testende Bild ist, \textit{testFolder\_path}, was der Pfad für einen Ordner darstellt, in dem die zu überprüfenden Bilder sind und \textit{similar\_count} mit dem angegeben werden kann wieviele der top Resultate ausgegeben werden sollen.
Die Funktion liest alle Bilder aus dem \textit{testFolder\_path} aus und für jedes Bild wird der SIFT Ähnlichkeitsscore in ein Dictionary gespeichert, wobei der Key für das Dictionary der Name des Bildes ist.
Abschließend wird das Dictionary nach dem SIFT Ähnlichkeitsscore sortiert und die Bilder mit dem höchste Ergebnis werden grafisch ausgegeben.
\begin{lstlisting}[frame=lines, caption=SIFT Testfunktion, captionpos=b, label = lst:sift_test_function, language=Python, showstringspaces=false]
def get_similar_images_sift(testImage_path, testFolder_path, similar_count):
    ...
    for image_name in sorted_image_similarity_dictionary:
        ...
        image_name=cv2.imread(str(image_name), cv2.COLOR_BGR2RGB)
        pairs.append([test_image,image_name])
        scores.append(sorted_image_similarity_dictionary[image_name])
    utils.plot_pairs(pairs,scores,similar_count)
\end{lstlisting}

Es folgt nun ein Beispiel wie ein Test aussieht.
Das zu testende Bild ist in Abbildung \ref{fig:sift_testimage} zu sehen.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=3.5cm]{images/sift_testimage.png}
	\caption{Testbild}
	\label{fig:sift_testimage}
\end{figure}
In Abbildung \ref{fig:sift_test_folder} ist ein Ausschnitt der zu vergleichenden Bilder zu sehen.
Insgesamt befinden sich in dem Ordner 997 Bilder.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=8cm]{images/sift_test_images.png}
	\caption{Ausschnitt von den zu vergleichenden Bildern}
	\label{fig:sift_test_folder}
\end{figure}
Abbildung \ref{fig:sift_test_result} veranschaulicht das Ergebnis.
In dem gezeigten Fall funktioniert der SIFT Algorithmus zuverlässig und findet die ähnlichsten Bilder mit einem Score von zwischen 14.19 und 23.75.
Ebenfalls ist zu festzuhalten, dass das Bild mit dem höchsten Ähnlichkeitsscore, welches dem Testbild nicht ähnelt, nur einen Wert von 6.76 hat.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=13cm]{images/sift_test_result.png}
	\caption{Ergebnis SIFT Vergleich}
	\label{fig:sift_test_result}
\end{figure}

\newpage
In Abbildung \ref{fig:sift_match_good} sind die Keypoints und Matches zwischen dem Testbild und dem laut SIFT ähnlichsten Bild dargestellt.
Die Matches befinden sich in diesem Fall hauptsächlich im Textbereich, wobei sich auch bei der Figur wenige befinden.
In Abbildung \ref{fig:sift_match_bad} sind die Keypoints und Matches zwischen dem Testbild und einem nicht ähnlichen Bild dargestellt und es ist zu sehen dass hier deutlich weniger Matches gefunden werden.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=8cm]{images/sift_match_good.png}
	\caption{SIFT Matching mit ähnlichem Bild}
	\label{fig:sift_match_good}
\end{figure}

\begin{figure}[htbp]
	\centering
		\includegraphics[height=7cm]{images/sift_match_bad.png}
	\caption{SIFT Matching mit unterschiedlichem Bild}
	\label{fig:sift_match_bad}
\end{figure}

\newpage
Ein weiterer durchgeführter Test war es negative und positive Paare mittels des SIFT Algorithmus zu vergleichen, um so mehr über SIFT und die Daten zu erfahren.
\begin{lstlisting}[frame=lines, caption=Hilfsfunktion zur Überprüfung von Paaren mittels SIFT, captionpos=b, label = lst:sift_test_pairs, language=Python, showstringspaces=false]
...
\end{lstlisting}
Mithilfe der Funktion aus Lisitng \ref{lst:sift_test_pairs} werden Paare aus einem Ordner mittels SIFT verglichen und jeweils die mit dem höchsten und niedrigsten grafisch ausgegeben.
Getestet wurde mit 200 positiven, sowie negativen Paaren.
In Abbildung \ref{fig:sift_test_negative_pairs} sind die Ergebnisse mit den negativen Paaren ersichtlich.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=13.5cm]{images/sift_test_negative_pairs_titled.png}
	\caption{Analyse negative Paare mit SIFT}
	\label{fig:sift_test_negative_pairs}
\end{figure}
Hier ist zu erkennen dass das Topresultat einen SIFT Score von 7.25 erzielt.
Auf den ersten Blick ist zu erkennen dass die Schriftart sehr ähnlich ist, und daraus das gute Resultat entstanden sein könnte.
Diese Hypothese konnte durch genaueres betrachten der Matches der Keypoints bestätigt werden (siehe Abbildung \ref{fig:sift_negative_pair_best_matches})
\begin{figure}[htbp]
	\centering
		\includegraphics[height=2cm]{images/sift_negative_pair_best_matches.png}
	\caption{Analyse Topresultat negative Paare}
	\label{fig:sift_negative_pair_best_matches}
\end{figure}

In Abbildung \ref{fig:sift_test_positive_pairs} sind die Ergebnisse der positiven Paare zu sehen.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=13.5cm]{images/sift_test_positive_pairs_titled.png}
	\caption{Analyse positive Paare mit SIFT}
	\label{fig:sift_test_positive_pairs}
\end{figure}
Die Topresultate erzielen einen SIFT Score von über 50, jedoch sind die Ergebnisse mit den niedrigsten Scores unter einem Score von 3.
Ebenfalls hat das niedrigste Paar einen SIFT Score von 0.
Bei genauerer Analyse hat sich herausgestellt dass der Deskriptor1 auf den Deskriptor2 30 gute Matches (Distanz <0.7), der Deskriptor2 auf Deskriptor1 8 gute Matches erkennt, jedoch keine davon bidirektional sind.

Schlussfolgerungen aus dieser Datenanalyse mit SIFT 
Durch diese Datenanalyse mittels SIFT ergeben sich folgende Schlussfolgerungen:
\begin{itemize}
	\item Ein Paar mit einem \textbf{SIFT Score > 7.25} kann als \textbf{positiv} angesehen werden.
	\item Ein Paar mit einem \textbf{SIFT Score < 7.25} kann jedoch \textbf{nicht} als \textbf{negativ} angesehen werden.
	\item Mit dem SIFT Algorithmus können positive Paare einen niedrigen Score erzielen.
\end{itemize}

\newpage
\section{Siamese Network}
\label{sec:siamese_implementation}
In diesem Abschnitt wird der Versuch für eine Bildähnlichkeitserkennung mittels eines siamesischen Netzwerkes beschrieben.
\subsection{Trainingsdaten}
Für das Simaesische Netzwerk werden Trainingsdaten benötigt.
Diese Trainingsdaten teilen sich in positive und negative Bildpaare auf.
Positive Paare sind jeweils zwei Bilder, welche sich ähnlich sind, wobei negative Paare zwei unterschiedliche Bilder beinhalten.
\subsubsection{Negative Paare}
Um negative Paare zu erstellen wurde eine Hilfsfunktion (siehe Listing \ref{lst:generate_random_negative_pairs}) geschrieben, welche zwei zufällige Bilder aus dem Datensatz hernimm
\begin{lstlisting}[frame=lines, caption=Hilfsfunktion zur Erstellung von negativen Paaren, captionpos=b, label = lst:generate_random_negative_pairs, language=Python, showstringspaces=false]
...
\end{lstlisting}
Die Hilfsfunktion nimmt die drei Parameter \textit{sourceFolder}, \textit{destinationFolder} und \textit{count} entgegen.
Es werden alle Bilder, welche sich in dem \textit{sourceFolder} befinden, geladen, und die Anzahl der bereits exisitirenden Paare aus dem \textit{destinationFolder} ermittelt.
Anschließend werden per Zufall zwei Bilder aus den geladenen Bildern ausgewählt und, sofern sie nicht dieselbe Datei sind, in den \textit{destinationFolder} kopiert.
Dabei erhalten sie die Namenskonvention \textit{neg\_IndexDesPaares\_1.gif} bzw. \textit{neg\_IndexDesPaares\_2.gif}.
Für das erste Paar sind die Namen beispielsweise \textit{neg\_1\_1.gif} und \textit{neg\_1\_2.gif}.
Aufgrund der Größe des Datensatzes ist es sehr unwahrscheinlich, dass dadurch fälschlicherweise positive Paare mitgeneriert werden, jedoch wurden die Paare anschließend auf Richtigkeit geprüft.
In Abbildung \ref{fig:negative_pairs} sind negative Paare zu sehen.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=10cm]{images/negative_pairs.png}
	\caption{Beispiele negative Paare}
	\label{fig:negative_pairs}
\end{figure}

\newpage
\subsubsection{Positive Paare}
Zu Beginn der Datenanalyse ist Aufgefallen dass es häufig der Fall ist, dass sich ähnliche Bilder in der Ordnerstruktur direkt nebeneinander befinden (siehe Abbildung \ref{fig:data_positive_pair} in rot markiert).
\begin{figure}[htbp]
	\centering
		\includegraphics[height=5cm]{images/data_positive_pair.png}
	\caption{Ausschnitt Datenstruktur positives Paar}
	\label{fig:data_positive_pair}
\end{figure}
Zu Beginn wurden die positiven Paare händisch generiert, dies beanspruchte jedoch sehr viel Zeit.
Mithilfe der gewonnen Erkentniss zum SIFT Algorithmus und der Datenanalyse wurde desshalb eine Hilfsfunktion geschrieben, mit der positive Paare generiert werden können.
\begin{lstlisting}[frame=lines, caption=Hilfsfunktion zur Überprüfung ob zwei Bilder ein positives Paar ergeben, captionpos=b, label = lst:is_positive, language=Python, showstringspaces=false]
def is_positive(filename1, filename2):
        ...
        return False
\end{lstlisting}
In der in Listing \ref{lst:is_positive} gezeigten Funktion werden zwei Dateinamen entgegengenommen und überprüft ob der SIFT Score für diese Dateien größer einem im Konfigfile beschriebenen Grenzwert entspricht.
Wenn dies der Fall ist so gibt die Funktion \textit{True} zurück, ansonsten \textit{False}.
Da es sich im vorher gezeigten Versuch um einen Test mit wenigen Daten (200 Paare) handelt, wurde auf den Grenzwert von 7.25 ein Sicherheitspuffer hinzugefügt.
Der Wert für \textit{config.SIFT\_SIMILARITY\_SCORE} wurde somit auf 9 gesetzt.
Hier gilt es anzumerken dass dieser Wert beliebig verändert werden kann.
Wird sich dieser Wert erhöht, dauert es durchschnittlich länger um positive Paare zu generieren, diese beinhalten dann jedoch weniger flasch positve Ergebnisse.
Ebenso gilt es wenn sich der Wert verringert, so dauert es durchschnittlich weniger lang um positive Paare zu generieren, jedoch beinhalten diese mehr falsch positve Ergebnisse.

\begin{lstlisting}[frame=lines, caption=Hilfsfunktion zur Generierung von positiven Paaren mittels SIFT, captionpos=b, label = lst:generate_random_positive_pairs, language=Python, showstringspaces=false]
def generate_random_positive_pairs(sourceFolder, destinationFolder, count):
    ...
    while(new_pairs_count < count):
        ...
        if(is_positive(file_to_check, file_neighbour2)):
            ...
            print('[INFO] currently generated pairs: ', new_pairs_count)
\end{lstlisting}
Die Funktion um positive Paare zu generieren (siehe Listing \ref{lst:generate_random_positive_pairs}) lädt alle Bilder aus dem im Parameter \textit{sourceFolder} enthaltenen Bilder und prüft anschließend Bildepaare mithilfe der in Listing \ref{lst:is_positive} beschriebenen Funktion auf Ähnlichkeit.
Dabei wird ein Bild per Zufall ausgewählt und mit seinen direkten Nachbarn verglichen, da die Datenanalyse ergeben hat, dass Nachbarn oft positive Paare darstellen (siehe Abbildung \ref{fig:data_positive_pair}).
Es benötigt nach der Generierung von positiven Paaren mittels SIFT eine händische Überprüfung der Paare, da es vorkommen kann dass falsch positive Paare dabei sind.
In Abbildung \ref{fig:positive_pairs} sind positive Paare zu sehen.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=10cm]{images/positive_pairs.png}
	\caption{Beispiele positive Paare}
	\label{fig:positive_pairs}
\end{figure}

\subsection{Siamese Network Implementierung}
Für das gesamte Projekt wurde eine Konfigurationsdatei erstellt, in der Werte wie z.B. der schon Beschriebene \textit{SIFT\_SIMILARITY\_SCORE} oder auch \textit{IMG\_WIDTH} und \textit{IMG\_HEIGHT} verwaltet werden können.
Hier wurden die Breite und Höhe Aufgrund von Speicherplatz auf die hälfte des in der Analyse ergebenen Mittelwertes gesetzt.
\begin{lstlisting}[frame=lines, caption=Konfigurationsdatei, captionpos=b, label = lst:config, language=Python, showstringspaces=false]
...
\end{lstlisting}
Um das Model für das siamese network zu builden wurde eine Hilfsfunktion geschrieben (siehe Listing \ref{lst:build_model}).
\begin{lstlisting}[frame=lines, caption=Siamese Network builden, captionpos=b, label = lst:build_model, language=Python, showstringspaces=false]
def build_model():
...
Mit der \textit{Input} Funktion wird ein Keras Tensor instanziert, welcher als Input für das Model verwendet wird.
Dabei wird die in der Konfigurationsdatei angegebene \textit{IMG\_SHAPE} als Shape angegeben.
Anschließend wird das ein CNN mittels der \textit{build\_cnn} Funktion konstruiert.
Diese Funktion ist in Listing \ref{lst:build_cnn} zu sehen.
Da es sich um ein siamese network handelt wird das CNN zwei mal instanziert und anschließend die euklidische Distanz angefügt.
Die euklidische Distanz berechnet die Differenz der Outputs der beiden CNNs, und somit die Ähnlichkeit.
Als Output-Layer wird ein vollständig verknüpftes Neuronales Netzwerk angefügt, welches mittels der Aktivierungsfunktion sigmoid einen Wert zwischen 0 und 1 ausgibt.
Das aus Listing  \ref{lst:build_model} entstandene Model ist in Abbildung \ref{fig:model_siamese} zu sehen.
\pagebreak
\begin{figure}[htbp]
	\centering
		\includegraphics[height=8cm]{images/model_siamese.png}
	\caption{Siamese Model}
	\label{fig:model_siamese}
\end{figure}

\begin{lstlisting}[frame=lines, caption=CNN builden, captionpos=b, label = lst:build_cnn, language=Python, showstringspaces=false]
def build_cnn(inputShape, embeddingDim=48):
	...
	model.summary()
	plot_model(model, to_file=os.path.sep.join([config.BASE_OUTPUT,'model.png']))
	# return the model to the calling function
	return model
\end{lstlisting}

Die in Listing  \ref{lst:build_cnn} gezeigte Funktion erzeugt ein CNN mit zwei Convolution Layern, die anschließend jeweils MaxPooling und ein Dropout haben.
Abschließend gibt es ein GlobalAveragePooling gefolgt von einem vollständig verknüpften neuronalen Netzwerk.
Das Model, welches aus dem Code aus in Listing  \ref{lst:build_cnn} erzeugt wird, ist in Abbildung \ref{fig:model_cnn}  zu sehen.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=17cm]{images/model_cnn.png}
	\caption{CNN Model}
	\label{fig:model_cnn}
\end{figure}

\subsection{Training des siamese netwok}
Zum Trainieren des siamese Networks wurde eine Funktion geschrieben, welche in Listing \ref{lst:train_siamese_network} zu sehen ist.
\begin{lstlisting}[frame=lines, caption=Funktion zum Trainieren des siamese networks, captionpos=b, label = lst:train_siamese_network, language=Python, showstringspaces=false]
...
\end{lstlisting}
Als Parameter werden die Pfade für die positiven und negativen Paare entgegengenommen, und ein Float, welcher angibt wie die Trainingsdaten ind Trainings- und Testdaten gesplittet werden soll.
Die positiven und negativen Paare werden mithilfe der Funktion \textit{make\_test\_and\_train\_pairs} geladen und gelabelt.
Positive Paare werden mit 1 gelabelt, und negative Paare mit 0 (siehe Abbildung \ref{fig:training_data}).
\begin{figure}[htbp]
	\centering
		\includegraphics[height=5cm]{images/training_data.png}
	\caption{CNN Model}
	\label{fig:training_data}
\end{figure}
Anschließend wird das simaesische Model geladen und für das Training konfiguriert.
In Listing \ref{lst:train_siamese_network} wird als Verlustfunktion \textit{binary\_crossentropy} gewählt, da es sich um ein Klassifikationsproblem von zwei Klassen handelt.
Die zwei Klassen sind dabei entweder positve, sprich ähnliche Bilder, oder negative, sprich unterschiedliche Bilder.
Mit der Funktion \textit{model.fit} wird das Model trainiert.
Mit \textit{batch\_size} wird angegeben wieviele Proben das Netzwerk nehmen soll, bevor es die internen Parameter wie Gewichte anpasst.
Mit \textit{epochs} wird angegeben wie oft die kompletten Trainingsdaten durchgearbeitet werden sollen.
Nach dem Trainieren des Netzwerkes werden die Gewichte des Models gespeichert 
Abschließend wird der Verlauf der Genauigkeit und des Verlustes über die Epochen grafisch ausgegeben.

\subsection{Ergebnisse}
Um das siamese Network zu testen wurde analog zur SIFT Testfunktion (siehe Listing \ref{lst:sift_test_function}) eine Hilfsfunktion geschrieben.
\begin{lstlisting}[frame=lines, caption=Siamese Testfunktion, captionpos=b, label = lst:get_similar_images_siamese, language=Python, showstringspaces=false]
def get_similar_images_siamese(testImage_path, testFolder_path, similar_count):
    ...
    utils.plot_pairs(pairs,scores,similar_count)
\end{lstlisting}
Die in Lisitng \ref{lst:get_similar_images_siamese} gezeigte Funktion agiert nach demselben Prinzip wie die Testfunktion für SIFT, jedoch wird zum ermitteln der Ähnlichkeit das siamese Network anstelle von SIFT genommen.

\subsubsection{1. Versuch}
Der erste Versuch wurde mit den in Kapitel \ref{sec:siamese_implementation} beschriebenem Netzwerk getätig.
Es wurden \textbf{50 Epochen} und eine \textbf{Batch Size} von \textbf{10} genommen.
Trainiert wurde das siamese Network mit jeweils \textbf{800 positiven} und \textbf{800 negativen} Paaren.
Die Genauigkeit und die Verluste pro Epoche während des Trainings ist in Abbildung \ref{fig:800_accuracy_loss} zu sehen.
\begin{figure}[htbp]
	\centering
		\includegraphics[height=10cm]{images/800_accuracy_loss.png}
	\caption{Training Loss und Accuracy mit 1600 Paaren}
	\label{fig:800_accuracy_loss}
\end{figure}
TBD.

\subsubsection{2. Versuch}
Es wurden \textbf{50 Epochen} und eine \textbf{Batch Size} von \textbf{10} genommen.
Trainiert wurde das siamese Network mit jeweils \textbf{2000 positiven} und \textbf{2000 negativen} Paaren.
TBD.
...
Siamese Network Datenanalyse und Trainingsdatenaufbereitung hat viel Zeit in anspruch genommen, leider wenig zeit für finetuning vom Netzwerk selbst
Anmerkung: Vergleich SIFT und EINE Form von siamese Network, man könnte siamese network finetunen und eventuell bessere Resultate erzielen.
Hinzufügen: Durchschnittliche Zeit pro Bildvergleich bei SIFT und siamese
\begin{table*}[htbp]
	\centering
